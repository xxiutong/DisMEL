...
from codes.model.modeling_dismel import DisMELEncoder, DisMELMatcher


class LightningForDisMEL(pl.LightningModule):
    def __init__(self, args):
        super(LightningForDisMEL, self).__init__()
        self.args = args
        self.save_hyperparameters(args)

        self.encoder = DisMELEncoder(args)
        self.matcher = DisMELMatcher(args)
    ...
    def training_step(self, batch):
        ...
        for k, v in batch.items():
            if k.startswith('ent_'):
                ...
                mention_batch[k] = v
        # entity_empty_image_flag = ent_batch.pop("empty_img_flag", None)

        answer_ids = mention_batch.pop("answer")
        mention_patch_mask = mention_batch.pop("mention_patch_mask")
        B = answer_ids.size(0)
        E = ent_batch['pixel_values'].size(0)

        mention_text_embeds, mention_image_embeds, \
            mention_text_seq_tokens, mention_image_patch_tokens = \
            self.encoder(**mention_batch)  # [B, D]

        ent_id_to_new = {}
        unique_indices = []

        for i in range(B):
            eid = int(answer_ids[i])
            if eid not in ent_id_to_new:
                ent_id_to_new[eid] = len(unique_indices)
                unique_indices.append(i)

        for i in range(E-B):
            unique_indices.append(B+i)

        new_ent_batch = {}
        for k, v in ent_batch.items():
            # v shape: [B, ...] → [E, ...]
            new_ent_batch[k] = v[unique_indices]

        entity_text_embeds, entity_image_embeds, \
            entity_text_seq_tokens, entity_image_patch_tokens = \
            self.encoder(**new_ent_batch)  # [E, D]

        logits, (text_logits, image_logits, image_text_logits) = \
            self.matcher(
                entity_text_embeds,
        ...
        labels = torch.empty(B, dtype=torch.long, device=logits.device)
        for i in range(B):
            labels[i] = ent_id_to_new[int(answer_ids[i])]
    ...
    def validation_step(self, batch, batch_idx):
        mention_patch_mask = batch.pop("mention_patch_mask")
        ...
        for idx in range(math.ceil(self.args.data.num_entity / chunk_size)):
            ...
            chunk_score, _ = self.matcher(chunk_entity_text_embeds, chunk_entity_text_seq_tokens,
                                          ...
                                          mention_image_embeds, mention_image_patch_tokens,mention_patch_mask)

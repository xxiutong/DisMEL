...
class TrainedModelForRankMatrix:
    ...
    def encode_mention_batch(self, batch):
        ...
        for m in batch:
            text = unquote(m["mentions"]) + " [SEP] " + m["sentence"] + " [SEP] " + m["desc"]
            texts.append(text)
        ...
        tok = self.tokenizer(texts, padding="max_length", truncation=True, max_length=77, return_tensors="pt")
    ...
    def preprocess(self):
        ...
        out_path = os.path.join(out_dir, "mel_pred_rank_dme.npy")
...
def main():
    ...
    # checkpoint_path = "/data/zyt/MMOR/runs/RichpediaMEL/version_5/checkpoints/epoch=3-step=392.ckpt"
    # checkpoint_path = "/data/zyt/MMOR/runs/RichpediaMEL/version_5/checkpoints/epoch=3-step=392.ckpt"
    checkpoint_path = "/data/zyt/MMOR/runs/RichpediaMEL/version_42/checkpoints/epoch=11-step=2340.ckpt"
